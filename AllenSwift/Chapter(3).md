...
﹒함수호출 코드, 계산코드, 클로저 코드 등 모두 가능(저장하려는 속성과 ⭐️리턴형⭐️만 일치하면 됨)
﹒메모리 공간이 없다가 처음 접근하는 순간에 (해당 속성만) 개별적으로 초기화 됨

사용하는 이유?
1) 메모리를 많이 차지할 때
2) 초기화 시점에 모든 속성들이 동시에 메모리 공간에 저장되므로 어떤 한 가지 속성이 다른 속성에 접근할 수 없다.
   하지만 지연 저장 속성을 이용하면 지연으로 저장된 속성은 먼저 초기화된 속성에 접근할 수 있게 된다.
```
<br>

__3) 계산 속성(Computed Properties)__
